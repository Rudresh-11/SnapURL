...
describe("Routes: /api/analytics", () => {
  ...
  test("GET /api/analytics/:id/allclicks -> 200 returns click rows", async () => {
    ...
    const res = await request(app).get(`/api/analytics/${url.id}/allclicks`).set("Authorization", `Bearer ${token}`)
  ...
  test("GET /api/analytics/:id/overview -> 200 returns overview (summary/daily/devices/countries/referrers)", async () => {
    ...
    const res = await request(app).get(`/api/analytics/${url.id}/overview`).set("Authorization", `Bearer ${token}`)
  ...
  test("GET /api/analytics/:id/clicksbydate -> 200 groups clicks by date", async () => {
    ...
    const res = await request(app).get(`/api/analytics/${url.id}/clicksbydate`).set("Authorization", `Bearer ${token}`)
  ...
  test("GET /api/analytics/:id/overview -> 404 for invalid url id", async () => {
    ...
    const res = await request(app).get("/api/analytics/999999999/overview").set("Authorization", `Bearer ${token}`)
